...
def predict_landing_risk(input_data):
    ...
    # Make predictions with dynamic scaling and consistent calculations
    try:
        # Get raw probabilities from classifier
        proba = models['classification'].predict_proba(input_df)[0]
        hard_landing_prob = proba[1]  # Probability of hard landing class
        
        # Get base risk score from regression model
        base_risk_score = min(max(models['regression'].predict(input_df)[0], 0), 100)  # Clamp between 0-100
        
        # Ensure consistency between the two metrics
        if hard_landing_prob > 0.5:  # If classifier predicts hard landing
            base_risk_score = max(base_risk_score, 70)  # Don't show low risk for predicted hard landings
            
        # Apply dynamic scaling based on critical parameters
        approach_speed = input_data.get('approach_speed_knots', 140)
        descent_rate = input_data.get('descent_rate_fpm', 700) 
        crosswind = input_data.get('crosswind_component_knots', 0)
        
        # Calculate adjustment factors
        speed_factor = 1 + (abs(approach_speed - 140) / 50)
        descent_factor = 1 + (abs(descent_rate - 700) / 300)
        crosswind_factor = 1 + (crosswind / 20)
        
        # Apply factors with reasonable bounds
        risk_score = min(
            base_risk_score * speed_factor * descent_factor * crosswind_factor,
            100
        )
...
def create_aircraft_visualization(pitch, roll, altitude, risk_level, approach_speed=140, descent_rate=700, wind_speed=0, wind_direction=0):
    ...
    # Ground plane (runway) - ultra visible size
    runway_length = 25000  # meters (can't miss it)
    runway_width = 1000  # meters (can't miss it)
    # Make runway markings extremely prominent
    centerline_width = 15  # meters wide
    edge_marking_width = 10  # meters wide
    
    # Make runway markings bold and clear
    runway_color = 'rgb(150,150,150)'  # Gray for runway
    centerline_color = 'rgb(255,255,0)'  # Yellow for centerline
    edge_color = 'rgb(255,255,255)'  # White for edges
    ...
    # Main runway surface - much more visible
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            camera=dict(
                eye=dict(x=0.5, y=0.5, z=1.2),  # Closer viewpoint
                up=dict(x=0, y=0, z=1),
