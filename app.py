...
# Add a button to start audio context for browser autoplay policy compliance
if 'audio_context_started' not in st.session_state:
    st.session_state.audio_context_started = False

if not st.session_state.audio_context_started:
    if st.sidebar.button("Start Audio"):
        st.session_state.audio_context_started = True

# View controls
st.sidebar.markdown("### View Controls")
if st.sidebar.button("Reset Camera", help="Reset to default view"):
...
with col1:
    ...
    if st.session_state.audio_warning and audio_enabled and st.session_state.audio_context_started:
        st.markdown("""
        <script>
            if (!window.audioContext) {
                ...
                window.oscillator.frequency.value = 440;
                ...
                window.beepInterval = setInterval(() => {
                    window.gainNode.gain.value = window.gainNode.gain.value > 0 ? 0 : 0.3;
                }, 500);
            }
        </script>
        """, unsafe_allow_html=True)
...
if st.session_state.prediction_history:
    ...
    with analysis_tabs[0]:
        ...
        **Risk Score Trend Analysis:**
        - Shows real-time risk changes during approach
        - 🟢 Below 30%: Safe approach (green zone)
        - 🟠 30-70%: Caution needed (orange warning)
        - 🔴 Above 70%: High risk - consider go-around (red danger)
        - ⭐ Markers: Go-around decisions
        - ● Markers: Landing attempts (color-coded by outcome)
        - Ideal: Maintain in green zone throughout approach
        """)
    ...
    with analysis_tabs[2]:
        ...
        if len(history_df) > 5:
            ...
            fig = go.Figure(data=[go.Scatter3d(
                # Clear flight path visualization with explanations
                x=history_df['timestamp'] / 1.5,  # Time scale adjusted
                y=history_df['localizer_deviation'] * 4,  # Clear lateral movement
                z=history_df['altitude'] / 30,  # Altitude scaling
                text=[
                    f"Time: {t:.1f}s | Lat: {y:.1f}m | Alt: {z:.1f}ft | Risk: {r}%" 
                    for t,y,z,r in zip(history_df['timestamp'], 
                                     history_df['localizer_deviation'],
                                     history_df['altitude'],
                                     history_df['risk_score'])
                ],
                mode='lines+markers',
                line=dict(
                    color='cyan',
            ...
            st.markdown("""
            **Flight Path Visualization Guide:**
            - 🟦 Blue line: Your aircraft's actual flight path
            - 🟩 Green dashed line: Ideal approach path (3° glideslope)
            - 🔴 Redder markers: Higher risk moments during approach
            - ⬆️ Vertical movement: Altitude changes
            - ↔️ Lateral movement: Localizer deviations
            - Hover over points to see time, position and risk score
            """)
