...
def generate_flight_data(num_rows=5000):
    ...
    # Enhanced risk score calculation with dynamic scaling
    def calculate_risk_score(row):
        # Base risk from landing rate
        base_risk = 1 / (1 + np.exp(-0.015 * (-row['landing_rate_fpm'] - 300)))
        
        # Adjust based on other critical factors
        weather_factor = 1.0
        if row['weather'] in ['Heavy Rain', 'Stormy']:
            weather_factor = 1.2
        elif row['weather'] in ['Windy', 'Foggy']:
            weather_factor = 1.1
            
        approach_factor = 1 + (abs(row['approach_speed_knots'] - 140) / 50)
        descent_factor = 1 + (abs(row['descent_rate_fpm'] - 700) / 300)
        crosswind_factor = 1 + (row['crosswind_component_knots'] / 20)
        
        # Combine factors with diminishing returns
        combined_risk = base_risk * min(
            weather_factor * approach_factor * descent_factor * crosswind_factor,
            1.5
        )
        
        return min(combined_risk * 100, 100)  # Cap at 100%
